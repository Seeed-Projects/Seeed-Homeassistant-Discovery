...
点击 `Leave Room` 并确认后，设备会分别发送 6 条关闭请求，其中包含预留的
空调实体。每个符合权限并可用的目标都会独立执行，最后显示全部成功、部分完成
或全部失败。可控制卡片的状态只跟随 Home Assistant 回传的真实状态变化。
...
左路开关、右路开关、窗户与电视实体已经出现在前面的显示订阅表中。空调安装
并准备好对应实体后，再选择预留空调实体。插件执行设备针对已选择实体发起的
操作。`Leave Room` 会分别请求每个目标，因此其中一个实体不可用或没有选择时，
其余已选择的设备仍会继续执行。温度和湿度会在屏幕上四舍五入并保留一位小数。
人员状态中的 `has one` 和 `no one` 会分别显示为 `Occupied` 和 `Vacant`。
