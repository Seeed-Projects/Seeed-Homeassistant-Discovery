...
- **Controls(控制)**：**设备**与**命令**下拉框用于选择当前设备并发送它的任意命令；从内置码库添加的空调会生成完整的温控卡片(电源、模式、温度、风速、扫风)。
- **Configuration(配置)**：**单击 / 双击 / 三击 / 长按**下拉框把命令绑定到对应触摸手势并同步到设备；**学习单击 / 双击 / 三击 / 长按**按钮采集一段红外信号并直接存入对应手势槽位，适合库中没有的冷门遥控器。
...
1. 点某个**学习**按钮，用遥控器对着 IR Mate 按目标键。通知会报告成功(含脉冲数与波形预览),或说明未采集到信号。
2. 打开 **Last learned IR signal** 传感器，即可看到采集到的脉冲数和属性里的完整 `timings` 数组；设备串口也会以 `IR learn OK` 打印同一数组。
3. 选择**命令**(或该手势绑定的命令)发射一次。**Last transmitted IR signal** 传感器与串口 `IR TX` 行会显示发出的波形。
4. 学习到的与发射出的 `timings` 应逐点一致，目标电器有响应即确认采集正确。
